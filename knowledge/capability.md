...
- **部署计划管理**：部署计划已成为业务单元下的可管理实体，支持列表、详情、新建、编辑、删除
- **部署计划组装规则**：每个部署计划必须绑定一个 CI 配置、一个 CD 配置、一个实例配置；三者必须归属于同一业务单元
- **部署计划命名规则**：部署计划名称在业务单元内唯一
- **平台管理入口**：DevOps 平台已在业务详情页提供“部署计划”管理页签，支持按名称搜索、分页查看、详情查看与增删改操作
