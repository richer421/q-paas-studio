...
DevOps 基建设施部署模块，用于快速部署 Jenkins、Harbor、GitLab 等 PaaS 平台前置依赖服务。
...
# 2. 部署服务
make infra-deploy SERVICE=jenkins          # 部署 Jenkins
make infra-deploy-all                      # 部署全部

# 3. 查看状态
make infra-status                          # 全部状态
make infra-status SERVICE=jenkins          # 单个状态

# 4. 销毁服务
make infra-destroy SERVICE=jenkins         # 销毁单个
...
```bash
make infra-deploy SERVICE=jenkins MODE=helm      # 强制 Helm
