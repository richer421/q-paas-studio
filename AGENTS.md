...
为减少无意义拆分、降低维护成本，新增以下固定规则：

1. **同一模块、同一业务实体的后端能力优先集中在一个实现文件**（例如同一个实体的 list/get/create/update/delete）。
2. 只有在满足“文件显著过大（如 > 400 行）且职责边界清晰稳定”时，才允许拆分文件；拆分后仍需按“稳定职责”分组，而不是按动词（如仅按 `*_list`、`*_crud`）机械拆分。
3. 如果出现命名冲突（如内部聚合查询与前端详情同名），优先通过**明确语义命名**解决（如 `GetOpenModelXxx` / `GetXxxDetail`），避免引入不必要的跨层或跨接口改动。
...
本项目本地调试统一使用**主仓根目录**的 `Makefile` 命令启动，不允许在子模块内自行选择启动方式。
...
1. 后端服务必须通过主仓命令启动：
   ...
   - `make dev-workflow`
   - `make dev-metahub`
   - 或 `make dev-backend MODULE=<module>`
2. 前端必须通过主仓命令启动：
   - `make dev-frontend`
3. 禁止在子模块目录内直接使用这些方式替代主仓命令：
   - `air`
   - `go run`
   - `./bin/<service> server`
   - `pnpm dev`
   - 以及其它绕过主仓 `Makefile` 的手动启动方式
4. 如果发现当前运行进程不是由主仓 `Makefile` 启动，应优先停止旧进程，再切回主仓统一命令重新启动。

目的：

- 保证所有模块的调试入口一致
- 保证热更新方式一致
- 避免出现“代码已修改但运行进程未按统一方式更新”的假象
